...
namespace Integration1C
{
    public class CreateOrUpdate1CLeadWithContacts
    {
        ...
        public CreateOrUpdate1CLeadWithContacts(int lead_id, AmoAccount acc, Log log)
        {
            _acc = acc;
            _lead_id = lead_id;
            _leadRepo1C = new();
        ...
        private static Lead1C Get1CLead(Lead lead, AmoAccount acc, Log log)
        {
                ...
                lead._embedded.companies.Any())
            {
                ...
                {
                    new CreateOrUpdate1CCompanyFromLead(lead.id, acc, log).Run();
        ...
        public void Run()
        {
            ...
            if (lead.custom_fields_values is not null &&
                ...
                try { _leadRepo1C.UpdateLead(Get1CLead(lead, _acc, _log)); }
