...
{
    internal class AbandonedCompaniesProcessor : AbstractReportProcessor, IReportProcessor
    {
        #region Definition
        ...
        private static bool CheckCompanyTasks(IEnumerable<AmoTask> tasks)
        {
            ...
            return tasks.Any(x => x.is_completed == false
                                  ...
                                  && DateTimeOffset.FromUnixTimeSeconds(x.complete_till).UtcDateTime.AddHours(3) < today.AddDays(28)
            ...
            List<(int, int)> dataranges = new()
{
        ...
        public override async Task Run()
        {
            ...
            List<(int, string, string, int, bool)> abandonedCompaniesResultsList = new();
            ...
            Parallel.ForEach(
                ...
                c => {
                    ...
                    {
                        GC.Collect();
                        _processQueue.UpdateTaskName(_taskId, $"Reports. AbandonedCompanies: processed {i} leads");
                    }
                    ...
                    if (CheckCompanyTasks(_compRepo.GetEntityTasks(c.id)))
                        return;

                    if (CheckEventsRecent(_compRepo.GetEntityEvents(c.id).Select(x => (x.type, (long)x.created_at)).ToList(), referenceDateTime, out contactTime))
                        return;
                    timeStamps.Add(contactTime);

                    if (CheckNotesRecent(_compRepo.GetEntityNotes(c.id).Select(x => (x.note_type, (long)x.created_at)).ToList(), referenceDateTime, out contactTime))
                        return;
                    timeStamps.Add(contactTime);
                    #endregion
                    ...
                    if (c._embedded.leads is not null)
                        ...
                        {
                            if (CheckLeadRecent(_leadRepo.GetById(lead.id), referenceDateTime, out contactTime, out completed))
                                return;
                            timeStamps.Add(contactTime);

                            if (CheckEventsRecent(_leadRepo.GetEntityEvents(lead.id).Select(x => (x.type, (long)x.created_at)).ToList(), referenceDateTime, out contactTime))
                                return;
                            timeStamps.Add(contactTime);

                            if (CheckNotesRecent(_leadRepo.GetEntityNotes(lead.id).Select(x => (x.note_type, (long)x.created_at)).ToList(), referenceDateTime, out contactTime))
                                return;
                            timeStamps.Add(contactTime);
                        }
                    #endregion
                    ...
                    if (c._embedded.contacts is not null)
                        ...
                        {
                            if (CheckEventsRecent(_contRepo.GetEntityEvents((int)contact.id).Select(x => (x.type, (long)x.created_at)).ToList(), referenceDateTime, out contactTime))
                                return;
                            timeStamps.Add(contactTime);

                            if (CheckNotesRecent(_contRepo.GetEntityNotes((int)contact.id).Select(x => (x.note_type, (long)x.created_at)).ToList(), referenceDateTime, out contactTime))
                                return;
                            timeStamps.Add(contactTime);
                        }
                    #endregion
            ...
            foreach (var ac in abandonedCompaniesResultsList)
            {
                ...
                var result = ProcessResult(ac.Item1, ac.Item2, ac.Item3, ac.Item4, ac.Item5);
